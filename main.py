...
def recommend(movie):
    ...
    movie_cast=[]
    movie_cast_poster=[]
    movie_cast.append(movie_info.iloc[index].cast)
...
if st.button('Show Recommendation'):
    ...
    index = movie_info[movie_info['title'] == selected_movie].index[0]
    st.markdown('Title - '+ movie_info.iloc[index].title )
    st.markdown('Overview -  ' + str(movie_info.iloc[index].overview))
    st.markdown('Rating -  ' + str(movie_info.iloc[index].vote_average) + "/10")
    st.markdown('Status -  ' + movie_info.iloc[index].status)
    st.markdown('Genre -  ' + str(movie_info.iloc[index].genres))
    st.markdown('Runtime - '+ str(movie_info.iloc[index].runtime)+" min")
    st.markdown('Director -  ' + str(movie_info.iloc[index].crew))
    st.markdown('Release Date - ' + movie_info.iloc[index].release_date)
    ...
    with col22:
        st.markdown(str(df1.iloc[index].sentiment))
        #st.markdown()
